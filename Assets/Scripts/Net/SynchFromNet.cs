...
public class SynchFromNet {
    ...
    public void Init()
    {
        ...
        changeFuncDic[SceneInfoKey.Talk] = ApplyTalkExp;
    }
    ...
    public void OnAckSceneObjInfoChange(SprotoTypeBase result)
    {
        ...
        for (int i = 0; i < len; i++)
        {
            ...
            for (int info_index = 0; info_index < info_len; info_index++)
            {
                ...
                Debug.Log("SynchFromNet OnAckSceneObjInfoChange key : "+cur_change_info.key);
    ...
    private void ApplyChangeInfoSceneChange(Entity entity, SprotoType.info_item change_info)
    {
        ...
        {
            long new_x = (long)Int64.Parse(strs[2]);
            long new_y = (long)Int64.Parse(strs[3]);
            long new_z = (long)Int64.Parse(strs[4]);
            //Debug.Log("ApplyChangeInfoSceneChange new_x"+new_x);
            Debug.Log("ApplyChangeInfoSceneChange uid : "+ (long)Int64.Parse(strs[1])+ new_x+new_y+new_z);
            Transform trans = SceneMgr.Instance.EntityManager.GetComponentObject<Transform>(entity);
            trans.localPosition = SceneMgr.Instance.GetCorrectPos(new Vector3(new_x/GameConst.RealToLogic, new_y/GameConst.RealToLogic, new_z/GameConst.RealToLogic));
            SceneMgr.Instance.EntityManager.SetComponentData(entity, new TargetPosition {Value = trans.localPosition});
            long uid = (long)Int64.Parse(strs[1]);
            SceneMgr.Instance.ChangeRoleUID(entity, uid);
            mainRoleGOE.name = "MainRole_"+uid;
        }
    }
        
    private void ApplyTalkExp(Entity entity, SprotoType.info_item change_info)
    {
            
            string[] strs = change_info.value.Split(',');
            int typeid = (int)Int64.Parse(strs[0]);
            long uid = (long)Int64.Parse(strs[1]);
            Debug.Log("ApplyTalkExp : " + change_info.value+typeid+uid);

            XLuaManager.Instance.ApplyTalkExp(typeid,uid);
            NetworkManager.GetInstance().onApplyTalkExp.Invoke(typeid,uid);
