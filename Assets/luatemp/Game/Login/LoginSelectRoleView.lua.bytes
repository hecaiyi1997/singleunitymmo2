		...
		local on_item_click = function ( )
			if item.data then
				for i,v in pairs(item.data) do
					print(i,"SetCurSelectRoleID",v)
				end
				print("SetCurSelectRoleID had_bind_click",item.data.role_id)
...
function LoginSelectRoleView:UpdateRoleMesh( role_vo )
	print("UpdateRoleMesh",role_vo.career,role_vo.body)
	local show_data = {
...
function LoginSelectRoleView:SetCurSelectRoleID( role_id )
	print("SetCurSelectRoleID UpdateRoleMesh",role_id,self.select_role_id)
