			...
			if not item then
				...
				item.OnUpdate = function(item)
					if not item.data then 
						print("item.OnUpdate   not item.data")
						return 
					end
					...
					print("SkillManager.GetInstance():GetSkillCD(item.data.skillID)",SkillManager.GetInstance():GetSkillCD(item.data.skillID))
					item:SetCDEndTime(SkillManager.GetInstance():GetSkillCD(item.data.skillID))
				end
				item.SetCDEndTime = function(item, cdEndTime)
					local curTime = Time:GetServerTime()  --当前时间 毫秒
					item.cdEndTime = cdEndTime            --结束时间
					item.cdRingTime = cdEndTime-curTime   --冷却时间长度
					item.countdown = item.countdown or item:AddUIComponent(UI.Countdown)
					item.cd_obj:SetActive(true)
					item.cd_img.fillAmount = 1
					item.countdown:CountdownByEndTime(item.cdEndTime/1000, function(leftTime)  --参数以秒为单位，需要把毫秒转秒
						if leftTime > 0 then
							item.left_time_txt.text = math.floor(leftTime)  
							local percent = 1000*leftTime/(item.cdRingTime)
